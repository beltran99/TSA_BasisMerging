...
def compute_min_com_merger(bases_set: list, mergers: list) -> tuple:
    """This function computes the CoM for a given list of bases mergers. It returns the merger with the minimum CoM and its value.

    Args:
        bases_set (list): Original set of bases, e.g., [1,2,3,4,5,6,7,8]
        mergers (list): List of mergers to compare, e.g., [[1,2],[3,4],[5,6],[7,8]]

    Returns:
        tuple: Merger with the minimum CoM and its value
    """    
    min_com = float('inf')
    min_merger = None
    ...
    for merger in mergers:
        acc_com = 0
        for x in merger:
            if len(x) > 1:  # compute the CoM if it is not a singleton
                x_com = dict(zip(bases_set, len(bases_set)*[0]))
